		...
		this._adminService.obtener_detalles_ordenes_estudiante(this.id, this.token).subscribe((response) => {
			...
			if (response.data != undefined) {
				...
				this.detalles.forEach((element:any) => {
					element.aniosup=new Date(new Date(element.idpension.anio_lectivo).setFullYear(new Date(element.idpension.anio_lectivo).getFullYear()+1)).getFullYear();
				});
...
*/
	...
	armado(){
		if(this.linkfact!=''){
			...
			this.registro.subtotal = parseFloat(this.pago.total_pagar.toFixed(2));
			...
			for (var k = 0; k < detalle_aux.length; k++) {
				...
				aux[17] = 0; //0
				aux[18] = "''"; //''
				if ( (this.pension[this.auxp].condicion_beca == 'Si'&& dtll.tipo>0 && dtll.tipo<=10 && dtll.valor==parseFloat(this.pension[this.auxp].val_beca).toFixed(2)&&dtll.abono==0)
				) {
					aux[19] = parseFloat(((parseFloat(parseFloat(this.pension[this.auxp].idanio_lectivo.pension).toFixed(2))*parseFloat(parseFloat(this.pension[this.auxp].desc_beca).toFixed(2)))/100).toFixed(2)); //descuento
